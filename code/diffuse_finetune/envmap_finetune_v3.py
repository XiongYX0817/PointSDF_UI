...
def finetune(**kwargs):
    ...
    flag = 'finetune_diffuse_albedo_v3_lr_%s_epochs_%s' % (str(lr), str(n_epochs))
    ...
    for data_index, (indices, model_input, ground_truth) in enumerate(eval_dataloader):
        ...
        for epoch in range(n_epochs):
            ...
            for dic in res:
                sg_diffuse_albedo_values = torch.ones_like(dic['points']).float().cuda()
           
                output = diffuse_albedo_layer(dic['differentiable_surface_points'])
                diffuse_albedo = output['sg_diffuse_albedo']

                sg_diffuse_albedo_values[dic['network_object_mask']] = diffuse_albedo

                dic.update({'sg_diffuse_albedo_values': sg_diffuse_albedo_values})
            ...
            model_outputs = utils.merge_output(ret, total_pixels, batch_size)
