...
def evaluate(**kwargs):
    ...
    # for ks in model.state_dict().keys():
    #     if ks.startswith('diffuse'):
    #         print(ks)

    
    ckpt_path = os.path.join('/data1/sr/code/PhySG/code/diffuse_finetune/models/finetune_diffuse_albedo_lr_0.0001_epochs_1000/model_params', str(kwargs['checkpoint']) + ".pth")
    ...
    # evaldir = evaldir + '_finetune_diffuse_albedo_v2_200_epochs'
    evaldir = evaldir + '_test_add_code'
    ...
    for dir in [rgb_images_dir, diffuse_rgb_images_dir, diffuse_albedo_images_dir]:
    ...
    for data_index, (indices, model_input, ground_truth) in enumerate(eval_dataloader):
        ...
        # print('total pixels: ', total_pixels, 'split length: ', len(split))
        ...
        points = model_outputs['points'].cpu().numpy()
        # o3d.visualization.draw_geometries([differentiable_surface_points])

        print(differentiable_surface_points.shape)

        # save points

        # np.save('/data1/sr/code/PhySG/3d_visualization/differentiable_surface_points/%s_differentiable_surface_points.npy' % out_img_name, differentiable_surface_points)
        # np.save('/data1/sr/code/PhySG/3d_visualization/points/%s_points.npy' % out_img_name, points)
