...
def finetune(**kwargs):
    ...
    model_save_dir = os.path.join('../cvpr23/exps', task, expname, flag)
    ...
    origin_image = imageio.imread(kwargs['origin_image']).astype(np.float32)[:, :, :3]
    ...
    for data_index, (indices, model_input, ground_truth) in enumerate(eval_dataloader):
        ...
        for epoch in range(n_epochs):
            ...
            total_pixels = differentiable_points.shape[0]
            split = utils.split_points({'differentiable_points': differentiable_points}, total_pixels)
            res = []

            for s in split:
                out = diffuse_albedo_layer(s['differentiable_points'])
                res.append({
                    'diffuse_albedo' : out['sg_diffuse_albedo']
                })

            model_outputs = utils.merge_points(res, total_pixels, batch_size=1)
...
def finetune_for_display(**kwargs):
    torch.set_default_dtype(torch.float32)
    ...
    for key in diffuse_albedo_layer_dict.keys():
        if key.startswith("diffuse"):
            tmp = ('envmap_material_network.' + key).split('.')
            tmp[-2] = 'layer_%s.0' % (int(tmp[-2]) // 2)
            diffuse_albedo_layer_dict[key] = model_dict[".".join(tmp[1:])]
        else:
            diffuse_albedo_layer_dict[key] = model_dict['envmap_material_network.'+key]
    ...
    edited_image = kwargs["edited_image"]
    edited_area = kwargs["mask_image"]
    ...
    rgb_gt = torch.from_numpy(edited_image).cuda().reshape((-1, 3))
    ...
    for data_index, (indices, model_input, ground_truth) in enumerate(eval_dataloader):
        ...
        for epoch in range(n_epochs):
            ...
            kwargs["msg_out"].print(f'Iter {epoch+1}/{n_epochs}, Loss: {loss.item():.4f}', replace=True)
        ...
        torch.save({"model_state_dict": model_dict}, "../ui/tmp.pth")

    return edited_3d_points
