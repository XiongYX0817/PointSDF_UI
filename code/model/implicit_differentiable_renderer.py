...
class IDRNetwork(nn.Module):
    ...
    def forward(self, input, assigned_color=None):
        # Parse model input
        ...
        if differentiable_surface_points.shape[0] > 0:
            ...
            ret = self.get_rbg_value(differentiable_surface_points, view_dirs, diffuse_rgb, assigned_color)
    ...
    def get_rbg_value(self, points, view_dirs, diffuse_rgb=None, assigned_color=None):
        ...
        sg_diffuse_albedo = sg_envmap_material['sg_diffuse_albedo']

        if assigned_color is not None:

                # print(ret['sg_diffuse_albedo'])
                # print(ret['sg_diffuse_albedo'].shape)
                assigned_color = [i / 255 for i in assigned_color]

                # print(assigned_color)
                assigned_diffuse_albedo = torch.tensor([assigned_color,] * sg_diffuse_albedo.shape[0]).to(torch.float32).cuda()

                # print(assigned_diffuse_albedo.shape)
                # exit()
                sg_diffuse_albedo = assigned_diffuse_albedo
        ...
        sg_ret = render_with_sg(lgtSGs=sg_envmap_material['sg_lgtSGs'],
                                ...
                                diffuse_albedo=sg_diffuse_albedo,
