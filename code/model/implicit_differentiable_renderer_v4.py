...
class IDRNetwork(nn.Module):
    ...
    def forward_dual_mlp(self, input, edited_pixels_coords):
        # Parse model input
        intrinsics = input["intrinsics"]
        uv = input["uv"]
        pose = input["pose"]
        ...
        num_samples = edited_pixels_idx.shape[0]

        depth = torch.ones((batch_size, num_samples)).cuda()
        x_cam = edited_pixels_coords[:, :, 0].view(batch_size, -1)
        y_cam = edited_pixels_coords[:, :, 1].view(batch_size, -1)
        z_cam = depth.view(batch_size, -1)

        edited_pixel_points_cam = lift(x_cam, y_cam, z_cam, intrinsics=intrinsics)

        # permute for batch matrix product
        edited_pixel_points_cam = edited_pixel_points_cam.permute(0, 2, 1)

        edited_world_coords = torch.bmm(p, edited_pixel_points_cam).permute(0, 2, 1)[:, :, :3]
        edited_ray_dirs = edited_world_coords - cam_loc[:, None, :]
        edited_ray_dirs_norm = F.normalize(edited_ray_dirs, dim=2)

        batch_size, edited_num_pixels, _ = edited_ray_dirs_norm.shape
        object_mask = torch.ones((edited_num_pixels, )).bool().cuda()

        self.implicit_network.eval()
        with torch.no_grad():
            edited_points, network_object_mask, edited_dists = self.ray_tracer(sdf=lambda x: self.implicit_network(x)[:, 0],
                                                                 cam_loc=cam_loc,
                                                                 object_mask=object_mask,
                                                                 ray_directions=edited_ray_dirs_norm)

        self.implicit_network.train()

        edited_points = (cam_loc.unsqueeze(1) + edited_dists.reshape(batch_size, edited_num_pixels, 1) * edited_ray_dirs_norm).reshape(-1, 3)

        edited_differentiable_surface_points = edited_points[network_object_mask]
        edited_pixels_idx = edited_pixels_idx[network_object_mask]

        edited_view_dirs = -edited_ray_dirs_norm.reshape(-1,3)[network_object_mask]
        ...
        differentiable_surface_points = self.merge_points(unedited_differentiable_surface_points, edited_differentiable_surface_points, split_idx)
        ...
        if differentiable_surface_points.shape[0] > 0:
            ...
            ret = self.get_rbg_value_dual_mlp(differentiable_surface_points_dict, view_dirs_dict, split_idx)
    ...
    def get_rbg_value_dual_mlp(self, differentiable_surface_points_dict, view_dirs_dict, split_idx, diffuse_rgb=None):
    ...
    def merge_points(self, unedited_points, edited_points, split_idx):

        length = unedited_points.shape[0] + edited_points.shape[0]
        edited_idx = split_idx['edited']
        unedited_idx = split_idx['unedited']
        ...
        points = torch.zeros((length, 3)).cuda()
        points[edited_idx] = edited_points
        points[unedited_idx] = unedited_points
