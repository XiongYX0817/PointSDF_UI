...
class PhySGEditingApp(labwidget.Widget):
    ...
    def render(self, poses, style, verbose=True, get_disps=False, update=False, update_cache=True, inds=None, use_cache=True):
        ...
        with torch.no_grad():
            ...
            if self.use_color_cache and use_cache and self.alphas and self.features and self.weights:
                ...
                images, disps, _, = render_path(poses, styles, self.hwfs, self.chunk, self.test_kwargs, nfs=self.nfs, alpha_cache=alpha_cache, feature_cache=feature_cache, weights_cache=weights_cache, verbose=False, cb=cb, update_cb=update_cb, detach=True)
            else:
                images, disps, _, alphas, features, weights = render_path(poses, styles, self.hwfs, self.chunk, self.test_kwargs, nfs=self.nfs, verbose=False, cb=cb, update_cb=update_cb, get_cached='color', detach=True)
                if update_cache:
                    self.alphas = alphas
    ...
    def create_color_dataset(self):
        ...
        pose = np.load(self.parentdir + "/" + str(args.instance) + "/" + args.editname + "/1_pose.npy")
        poses = torch.from_numpy(np.array([pose])).to(poses.device)
    ...
    def optimize(self):
        ...
        if len(self.hwfs[0]) == 3:
            H, W, f = self.hwfs[0]
        else:
            H, W, fx, fy = self.hwfs[0]
            f = [fx, fy]
        # print(self.hwfs[0])
