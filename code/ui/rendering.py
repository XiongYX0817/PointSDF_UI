...
def render(H=None, W=None, focal=None, style=None, alpha=None, feature=None, weights=None, chunk=1024 * 32, rays=None, c2w=None, ndc=False,
           near=0., far=1., use_viewdirs=False, viewdirs_reg=None, detach=False, **kwargs):
    """Render rays
    Args:
    ...
    # Render and reshape
    all_ret = batchify_rays(rays, style, alpha, feature, weights, chunk, viewdirs_reg=viewdirs_reg, detach=detach, **kwargs)
...
def render_path(render_poses, styles, hwfs, chunk, render_kwargs, nfs=None, gt_imgs=None, alpha_cache=None, feature_cache=None, weights_cache=None, savedir=None, maximum=1000, get_cached=None, get_rgbs=False, verbose=True, cb=None, update_cb=None, detach=False):
    ...
    for i, (c2w, style) in enumerate(iterator):
        ...
        rgb, disp, acc, additional = render(H, W, focal, style=style, chunk=chunk, weights=weight, c2w=c2w[:3, :4], alpha=alpha, feature=feature, detach=detach, **render_kwargs)
...
def batchify_rays(rays_flat, style, alpha, feature, weights, chunk=1024 * 32, N_samples=64, N_importance=0, viewdirs_reg=None, detach=False, **kwargs):
    ...
    # all_ret = {k: torch.cat(all_ret[k], 0).detach().cpu() for k in all_ret}
